  ...
  render(element: HTMLElement) {
    ...
    for (let i = 0; i < this.data.length; i++) {
      ...
      for (let j = 0; j < row.length; j++) {
        ...
        if (i == 0 && j == 4) row[j].addTopRightCurve(30);
        if (i == 4 && j == 0) row[j].addBottomLeftCurve(30);
        if (i == 4 && j == 4) row[j].addBottomRightCurve(30);
...
export class BingoCardFreeSpace extends BingoCardItem {
  ...
  constructor(imageUrl: string, altText: string, artistName: string, sourceUrl: string, streched: boolean = false) {
    super(
      "Free Space",
      `the stream starts (selected automatically)<br/><br/>Art credit: <a href="${sourceUrl}" target="_blank">${artistName}</a>`,
      BingoItemCategory.Chat,
      true,
      false,
    );
