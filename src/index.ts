...
(
  document.getElementsByClassName("bingo-container")[0] as HTMLElement
).addEventListener(
  "contextmenu",
...
setTimeout(() => {
  document.getElementById(
    "bingo-artwork-credit-below"
  )!.innerHTML = `Center artwork credit: <a href="${freeSpace.sourceUrl}" target="_blank">${freeSpace.artistName}</a>`;
});

if (window.localStorage.getItem("board-state") != null) {
  let data = JSON.parse(
    window.localStorage.getItem("board-state") as string
  ) as SavedCard;
...
function generateNewBoard() {
  fetch("/data/boards/neuro_zoo.json")
    ...
    .then(
      (prompts: { name: string; description: string; category: string }[]) => {
