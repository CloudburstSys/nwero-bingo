export default class Emote {
    ...
    constructor(name: string) {
        this.name = name;
        this.url = `/emotes/${this.name}.webp`;
    }
    ...
    static async fromString(str: string): Promise<Emote | null> {
        ...
        if (result != null) {
            let isReal = await fetch(`/emotes/${result[1]}.webp`).then(res => res.status != 200 ? false : true);

            if (isReal)
                return new this(result[1]);
        }
